...
nets = dict(
    ...
    dogecoin=math.Object(
     ...
     fastcoinsha=math.Object(
        PARENT=networks.nets['fastcoinsha'],
        SHARE_PERIOD=30, # seconds target spacing
        CHAIN_LENGTH=12*60*60//30, # shares
        REAL_CHAIN_LENGTH=12*60*60//30, # shares
        TARGET_LOOKBEHIND=20, # shares coinbase maturity
        SPREAD=20, # blocks
        IDENTIFIER='F0F1F2D3B2F68CF9'.decode('hex'),
        PREFIX='F0F3D4F541C11DF9'.decode('hex'),
        P2P_PORT=7982,
        MIN_TARGET=0,
        MAX_TARGET=2**256//2**32 - 1,
        PERSIST=False,
        WORKER_PORT=8982,
        BOOTSTRAP_ADDRS='emiko.dyndns.info 192.227.238.236 pool.broketech.pw'.split(' '),
        ANNOUNCE_CHANNEL='#p2pool-alt',
        VERSION_CHECK=lambda v: True,
    ),
chikun=math.Object(
        PARENT=networks.nets['chikun'],
        SHARE_PERIOD=60, # seconds target spacing
        CHAIN_LENGTH=12*60*60//30, # shares
        REAL_CHAIN_LENGTH=12*60*60//30, # shares
        TARGET_LOOKBEHIND=20, # shares coinbase maturity
        SPREAD=20, # blocks
        IDENTIFIER='FAFAFAD4B4F68AB0'.decode('hex'),
        PREFIX='FAFAD5A541C00DF9'.decode('hex'),
        P2P_PORT=50012,
        MIN_TARGET=0,
        MAX_TARGET=2**256//2**32 - 1,
        PERSIST=False,
        WORKER_PORT=50011,
        BOOTSTRAP_ADDRS='192.210.146.143'.split(' '),
        ANNOUNCE_CHANNEL='p2pool-broke',
        VERSION_CHECK=lambda v: True,
    ),
 parallelcoinscrypt=math.Object(
        PARENT=networks.nets['parallelcoinscrypt'],
        SHARE_PERIOD=30, # seconds target spacing
        CHAIN_LENGTH=12*60*60//30, # shares
        REAL_CHAIN_LENGTH=12*60*60//30, # shares
        TARGET_LOOKBEHIND=30, # shares coinbase maturity
        SPREAD=20, # blocks
        IDENTIFIER='F1F1D2D3A2F38CF9'.decode('hex'),
        PREFIX='F0F0D4F451C12DF0'.decode('hex'),
        P2P_PORT=13049,
        MIN_TARGET=0,
        MAX_TARGET=2**256//2**20 - 1,
        PERSIST=False,
        WORKER_PORT=13050,
        BOOTSTRAP_ADDRS='23.94.26.93'.split(' '),
        ANNOUNCE_CHANNEL='',
        VERSION_CHECK=lambda v: True,
    ),
 parallelcoinsha=math.Object(
        PARENT=networks.nets['parallelcoinsha'],
        SHARE_PERIOD=30, # seconds target spacing
        CHAIN_LENGTH=12*60*60//30, # shares
        REAL_CHAIN_LENGTH=12*60*60//30, # shares
        TARGET_LOOKBEHIND=20, # shares coinbase maturity
